      ...
      content: [
        ...
        // Brand title
        {
          columns: [
            {
              stack: [
                { text: 'HOPKID HRM  •  ATTENDANCE REPORT', fontSize: 8, bold: true, color: PRIMARY_COLOR, margin: [0, -68, 0, 2] },
                { text: reportTitle, fontSize: 16, bold: true, color: 'white', margin: [0, 0, 0, 2] },
                { text: isSingle ? `Employee Code: ${emp0.employeeCode}  •  Dept: ${(emp0 as any).department?.name || 'Operations'}  •  Store: ${emp0.office?.name || 'Headquarters'}` : `Organisation-wide attendance summary  •  Month: ${monthLabel}`, fontSize: 9, color: '#94A3B8' },
              ],
            },
            {
              stack: [
                { text: `Month: ${monthLabel}`, fontSize: 10, bold: true, color: 'white', alignment: 'right', margin: [0, -68, 0, 4] },
                { text: `Generated By: ${generatedBy}`, fontSize: 8, color: '#94A3B8', alignment: 'right' },
        ...
        // ── SUMMARY STAT CARDS
        {
          columns: [
            {
              stack: [
                { text: totalCardVal1, fontSize: 20, bold: true, color: PRIMARY_COLOR, alignment: 'center' },
                { text: totalCardLbl1, fontSize: 7, bold: true, color: '#64748B', alignment: 'center', margin: [0, 2, 0, 0] },
              ],
              alignment: 'center',
            },
            {
              stack: [
                { text: totalCardVal2, fontSize: 20, bold: true, color: '#059669', alignment: 'center' },
                { text: totalCardLbl2, fontSize: 7, bold: true, color: '#64748B', alignment: 'center', margin: [0, 2, 0, 0] },
              ],
              alignment: 'center',
            },
            {
              stack: [
                { text: totalCardVal3, fontSize: 20, bold: true, color: '#D97706', alignment: 'center' },
                { text: totalCardLbl3, fontSize: 7, bold: true, color: '#64748B', alignment: 'center', margin: [0, 2, 0, 0] },
              ],
              alignment: 'center',
            },
            {
              stack: [
                { text: totalCardVal4, fontSize: 20, bold: true, color: '#DC2626', alignment: 'center' },
                { text: totalCardLbl4, fontSize: 7, bold: true, color: '#64748B', alignment: 'center', margin: [0, 2, 0, 0] },
              ],
              alignment: 'center',
            },
            {
              stack: [
                { text: totalCardVal5, fontSize: 20, bold: true, color: '#6366F1', alignment: 'center' },
                { text: totalCardLbl5, fontSize: 7, bold: true, color: '#64748B', alignment: 'center', margin: [0, 2, 0, 0] },
              ],
              alignment: 'center',
            },
          ],
        ...
        { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 770, y2: 0, lineWidth: 1, lineColor: '#E2E8F0' }], margin: [0, 0, 0, 14] },
        ...
        ...(!isSingle
          ? [
              {
                columns: [
                  { text: 'Employee Attendance Summary', bold: true, fontSize: 11, color: '#0F172A' },
                  { text: `${employees.length} Employee${employees.length !== 1 ? 's' : ''}`, fontSize: 9, color: '#64748B', alignment: 'right' },
                ],
                margin: [0, 0, 0, 8],
              },
              {
                table: {
                  headerRows: 1,
                  dontBreakRows: true,
                  widths: [160, 65, 140, 55, 55, 55, 55, 55, '*'],
                  body: [
                    [
                      { text: 'Employee Name', style: 'colHeader' },
                      { text: 'Code', style: 'colHeader' },
                      { text: 'Department / Office', style: 'colHeader' },
                      { text: 'Present', style: 'colHeader', alignment: 'center' },
                      { text: 'Late', style: 'colHeader', alignment: 'center' },
                      { text: 'Half Day', style: 'colHeader', alignment: 'center' },
                      { text: 'Absent', style: 'colHeader', alignment: 'center' },
                      { text: 'Leave', style: 'colHeader', alignment: 'center' },
                      { text: 'Attendance Rate', style: 'colHeader', alignment: 'center' },
                    ],
                    ...employeeData.map((empData, i) => [
                      ...
                      { text: empData.employee.employeeCode, fontSize: 8, color: '#64748B', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                      { text: `${(empData.employee as any).department?.name || '—'} / ${empData.employee.office?.name || 'Main'}`, fontSize: 8, color: '#64748B', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                      { text: empData.present.toString(), fontSize: 8, alignment: 'center', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                      { text: empData.late.toString(), fontSize: 8, alignment: 'center', color: empData.late > 0 ? '#D97706' : '#64748B', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                      { text: empData.halfDay.toString(), fontSize: 8, alignment: 'center', color: empData.halfDay > 0 ? '#4F46E5' : '#64748B', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                      { text: empData.absent.toString(), fontSize: 8, alignment: 'center', color: empData.absent > 0 ? '#DC2626' : '#64748B', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                      { text: empData.leave.toString(), fontSize: 8, alignment: 'center', color: empData.leave > 0 ? '#7C3AED' : '#64748B', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                      { text: `${empData.attendanceRate}%`, fontSize: 8, alignment: 'center', bold: true, color: empData.attendanceRate >= 90 ? '#059669' : empData.attendanceRate >= 75 ? '#D97706' : '#DC2626', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                    ]),
                  ],
                },
                layout: {
        ...
        ...employeeData.map((empData, index) => {
          ...
          return [
            ...
            empData.attendances.length > 0
                  ...
                  table: {
                    ...
                    body: [
                      ...
                      ...empData.attendances.map((att, i) => [
                        ...
                        { text: (empData.employee as any).department?.name || 'Operations', fontSize: 8, color: '#64748B', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                        { text: formatTime(att.checkIn), fontSize: 8, color: '#374151', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
                        { text: formatTime(att.checkOut), fontSize: 8, color: '#374151', fillColor: i % 2 === 0 ? '#F8FAFC' : 'white' },
              ...
              : { text: 'No attendance records logged for this month.', fontSize: 9, color: '#94A3B8', italics: true, alignment: 'center', margin: [0, 16, 0, 16] }
          ];
        }).flat()
      ],
